...
import com.zensar.utilities.DriverHandler;
...
public class ScenarioState implements StringLiterals {
	...
	public ScenarioState() {
		...
		try {
			for(int i = 0; i < stackTraceElements.length ; i++) {
				...
				if(fileName.equals("TestNGCucumberRunner.java")) {
					runnerName = stackTraceElements[i+1].getFileName().split("\\.")[0];
					break;
				}
			}
		}catch (Exception e) {
			runnerName = "Not Found";
			System.err.println("[ERROR] Problem while finding out runner name");
			e.printStackTrace();
		}
	...
	RemoteWebDriver getMobileDriver() {
		return driverHandler.getMobileDriver();
	}
	RemoteWebDriver getWindowsDriver() {
		return driverHandler.getWindowsDriver();
	}
